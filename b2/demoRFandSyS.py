...
# Thay đổi từ DecisionTreeClassifier sang RandomForestClassifier
...
spark = SparkSession.builder \
    .appName("SysSamplingRandomForestBankData") \
...
try:
    ...
    # ------------------------------------------------------------------
    # HÀM HUẤN LUYỆN VÀ ĐÁNH GIÁ MÔ HÌNH RANDOM FOREST TRÊN MỘT MẪU DỮ LIỆU
    # ------------------------------------------------------------------
    def train_and_evaluate_random_forest(rdd_sample, experiment_id,
                                         rf_num_trees=20, rf_max_depth=10, rf_max_bins=32,
                                         train_test_seed=None):
        """
        Chuyển RDD mẫu thành DataFrame, tiền xử lý, huấn luyện Random Forest và đánh giá.

        Args:
            rdd_sample (RDD): RDD chứa các đối tượng Row của mẫu dữ liệu.
            experiment_id (int): ID của lần chạy thử nghiệm.
            rf_num_trees (int): Số cây trong Random Forest.
            rf_max_depth (int): Chiều sâu tối đa của mỗi cây.
            rf_max_bins (int): Số lượng bin tối đa.
            train_test_seed (int, optional): Seed cho randomSplit và Random Forest.
        ...
        # Thay thế DecisionTreeClassifier bằng RandomForestClassifier
        rf = RandomForestClassifier(labelCol="indexedLabel", featuresCol="features",
                                    numTrees=rf_num_trees, maxDepth=rf_max_depth,
                                    maxBins=rf_max_bins, seed=train_test_seed)
        stages.append(rf)
    ...
    print(f"\nBắt đầu chạy {num_experiments} thử nghiệm song song (lấy mẫu SyS + Random Forest)...")
    ...
    for i in range(num_experiments):
        ...
        # Chạy hàm huấn luyện và đánh giá mô hình Random Forest trên mẫu SyS này
        result = train_and_evaluate_random_forest(
            current_sys_sample_rdd,
            experiment_id=i + 1,
            rf_num_trees=50,  # Có thể tăng số cây để tăng độ chính xác
            rf_max_depth=10,  # Độ sâu tối đa của mỗi cây
